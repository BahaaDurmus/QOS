...
class StreamClient:
    ...
    # ── Video Writer ──────────────────────────────────────────────────────────
    def _make_writer(self, w: int, h: int, suffix: str) -> cv2.VideoWriter:
        ts   = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = self.out_dir / f"{suffix}_{ts}.mp4"
        fourcc = cv2.VideoWriter_fourcc(*"mp4v")
        writer = cv2.VideoWriter(str(path), fourcc, self.out_fps, (w, h))
        if not writer.isOpened():
            raise RuntimeError(f"VideoWriter açılamadı: {path}")
        print(f"[Client] Video yazıcı: {path}", flush=True)
        return writer, path
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # İlk kare: boyutları belirle ve writer'ları oluştur
                if frame_bgr is None:
                    # Çözümleme tamamen başarısız → placeholder kullan
                    if placeholder_bgr is None:
                        continue  # daha boyut bilmiyoruz, atla
                ...
                tensor = _frame_to_tensor(frame_bgr)
                window.append(tensor)
                if len(window) > self.WINDOW_SIZE:
                    window.pop(0)

                # VSR Restorasyonu
                restored_bgr = frame_bgr  # default: ham kare
                if not self.no_vsr and self.model is not None and len(window) == self.WINDOW_SIZE:
                    try:
                        restored_bgr = vsr_restore(self.model, window, self.device)
                        self._restored_frames += 1
                    except Exception as e:
                        print(f"[Client] VSR hatası kare {frame_id}: {e}", flush=True)

                # Onarılmış videoyu yaz
                if restored_writer:
                    restored_writer.write(restored_bgr)

                # Görüntüle
                if self.display:
                    compare = np.hstack([frame_bgr, restored_bgr])
                    label_h = 30
                    canvas = np.zeros((h + label_h, w * 2, 3), dtype=np.uint8)
                    canvas[label_h:, :] = compare
                    cv2.putText(canvas, "Ham Akis (Bozuk)", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (100, 200, 255), 2)
                    cv2.putText(canvas, "VSR Onarim", (w + 10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (100, 255, 100), 2)
                    cv2.imshow("QoS Stream – Sol: Ham  |  Sag: VSR Onarim", canvas)
                    if cv2.waitKey(1) & 0xFF == ord("q"):
                        print("[Client] Kullanıcı 'q' ile çıkış yaptı.", flush=True)
                        break
            ...
            print("\n[Client] Kullanıcı tarafından durduruldu.", flush=True)
        finally:
            receiver.stop()
            if restored_writer:
